...
public class CameraRulerView extends View {
    ...
    public CameraRulerView(Context context, Toolbar toolbar, MainActivity parent) {
        super(context);
    ...
    protected void executeTouch(MotionEvent event) {
        if (event.getAction() == ACTION_UP || event.getAction() == ACTION_CANCEL) {
            ...
            if (ctxStatus == MainActivity.Status.REFERENCE) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (measure != null) {
            ...
            if (measure instanceof Line) {
                ...
                toolbar.setSubtitle(getResources().getString(R.string.length) + " " + length*15 + unit);
            } else {
                float area = 0;
                String unit = unitOfMeasurement;
                ...
                if (measure instanceof Polygon && ((Polygon) measure).isSelfIntersecting()) {
                    ...
                    toolbar.setSubtitle(getResources().getString(R.string.area) + " " + area*170 + unit + "²");
                }
