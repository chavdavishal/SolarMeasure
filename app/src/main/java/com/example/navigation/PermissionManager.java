...
public class PermissionManager {
    ...
    void handlePermissionResult(Activity activity,int requestCode, String[] permissions,int[] grantResults){
        ...
        if (grantResults.length > 0) {
            for (int i = 0; i < grantResults.length; i++) {
                if (grantResults[i] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    //Toast.makeText(activity, "Permission ok ok.", Toast.LENGTH_SHORT).show();
                }
            }
    ...
    private void showPermissionRational(Activity activity, int requestCode){
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M){
            if (ActivityCompat.shouldShowRequestPermissionRationale(activity,
                ...
                showMessageOkCancle("you need to allow access to the permission(s)!",
    ...
    void showMessageOkCancle(String msg,DialogInterface.OnClickListener onClickListener){
